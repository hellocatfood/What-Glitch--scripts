...
echo -e "\033[33mWhat Glitch? tga. \033[0m"
...
ffmpeg -i $file -sameq -r $framerate out_%d.tga
...
echo -e "\033[33mConverting files to .bmp's as ffmpeg doesn't like these glitched files \033[0m"
#ffmpeg doesn't seem to like these glitch .tga's so we convert them to .bmp's
gimp -n -i -b - <<EOF
...
ffmpeg -i out_%d.bmp -sameq -vcodec huffyuv -r $framerate ../outfile_tga_"$rand".mkv

#remove the temporary directory
cd ../
rm -rf temp_$rand/
echo -e "\033[33mJob done. Check for outfile_tga_"$rand".mkv \033[0m"
